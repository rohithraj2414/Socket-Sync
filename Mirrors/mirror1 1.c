...
#define PORT 8088  //Port number for mirror1
    ...
    if (size1 <= size2 && size1 >= 0 && size2 >= 0) {
        ...
        if (system(find_command) != -1) {
            ...
            if (stat(tempTarPath, &st) == 0 && st.st_size > 0) {
                ...
                send_file_to_client(new_socket, tempTarPath);
...
// length of the destination path, including the directory path and the filename "temp.tar.gz".
    ...
    char *extensions = strtok(buffer + 6, " ");
 
    // Split the extension list into individual extensions
    char *extension1 = strtok(extensions, " ");
    char *extension2 = strtok(NULL, " ");
    char *extension3 = strtok(NULL, " ");
 
    // Check if the extension list is valid (at least one extension and at most three)
    if (extension1 != NULL && (extension2 == NULL || extension3 == NULL)) {
        // Forming the command to find and tar files with specified extensions
        char find_command[512];
        // Ensure the temp file does not already exist or can be overwritten
        unlink(tempTarPath);  // Attempt to delete the old file if it exists
 
        // Construct the find command based on the number of extensions provided
        if (extension2 == NULL) {
            sprintf(find_command, "find ~ -type f -iname '*.%s' -print0 | tar --null -czvf %s -T -", extension1, tempTarPath);
        } else if (extension3 == NULL) {
            sprintf(find_command, "find ~ -type f -iname '*.%s' -o -iname '*.%s' -print0 | tar --null -czvf %s -T -", extension1, extension2, tempTarPath);
        } else {
            sprintf(find_command, "find ~ -type f -iname '*.%s' -o -iname '*.%s' -o -iname '*.%s' -print0 | tar --null -czvf %s -T -", extension1, extension2, extension3, tempTarPath);
        }
        ...
        send(new_socket, "Invalid extension list.\n", 24, 0);
    }
}
